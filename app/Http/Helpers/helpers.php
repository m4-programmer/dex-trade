...
function displayNavItems():object{
...
function getPaymentEndDate($array='')
{
     $duration = explode(' ', $array);
    // check if duration is in year
    if (strtolower($duration[1]) == 'years' or strtolower($duration[1]) == 'year' ) {
        $updated_at = Carbon::now()->addYears($duration[0]);
    }
